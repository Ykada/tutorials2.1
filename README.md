# tutorials2.1

## Les 1 / Les 2 - Introductie en Interfacenavigatie

**Opdracht:**  
Toont aan hoe je aan de slag gaat met Unity, de interface verkent en initiële projectbestanden opent.

![Les 1/Les 2 GIF](https://private-user-images.githubusercontent.com/120028703/488777352-42c50637-b821-4482-a1ff-0106ea4fe84b.gif?jwt=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NjI0MTcxOTYsIm5iZiI6MTc2MjQxNjg5NiwicGF0aCI6Ii8xMjAwMjg3MDMvNDg4Nzc3MzUyLTQyYzUwNjM3LWI4MjEtNDQ4Mi1hMWZmLTAxMDZlYTRmZTg0Yi5naWY_.aaVPnR2bb9TywPtttqggxePi4gPOpSuBcbpyWvkCkvQ)

---

## Les 2 - Objectplaatsing en -manipulatie

**Opdracht:**  
Toont hoe je objecten aan de scene toevoegt, ze verplaatst en basiseigenschappen in Unity gebruikt.

![Les 2 GIF](https://private-user-images.githubusercontent.com/120028703/496554015-d9ee5a84-15cd-45d3-ac3b-e9941af78634.gif?jwt=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NjI0MTcxOTYsIm5iZiI6MTc2MjQxNjg5NiwicGF0aCI6Ii8xMjAwMjg3MDMvNDk2NTU0MDE1LWQ5ZWU1YTg0LTE1Y2QtNDVkMy1hYzNiLWU5OTQxYWY3ODYzNC5naWY_.JrSIh53a8SvErwlZywZ-wl0cQG-PaGZX12FvR_ouBUg)

---

## Les 3 - Componenten toevoegen en configureren

**Opdracht:**  
Omvat het toevoegen van componenten aan objecten (bijv. scripts, physics, renderers) en het aanpassen van instellingen.

![Les 3 GIF](https://private-user-images.githubusercontent.com/120028703/499278714-8d2c4831-ed81-4a5b-8efe-db0af462cf9d.gif?jwt=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NjI0MTcxOTYsIm5iZiI6MTc2MjQxNjg5NiwicGF0aCI6Ii8xMjAwMjg3MDMvNDk5Mjc4NzE0LThkMmM0ODMxLWVkODEtNGE1Yi04ZWZlLWRiMGFmNDYyY2Y5ZC5naWY_.NtU9QbjqVbaRCA6kxNyvpbblgtSRWEeZn3mRcHvPgsE)

---

## Les 4 - Basisaanstellingen & Scripting

**Opdracht:**  
Introduceert hoe je scripts schrijft, deze aan game objecten toewijst en eenvoudige code in Unity test.

![Les 4 GIF](https://private-user-images.githubusercontent.com/120028703/510134921-6c3fa540-0a2d-4a1d-a40e-319b5bf810d4.gif?jwt=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NjI0MTcxOTYsIm5iZiI6MTc2MjQxNjg5NiwicGF0aCI6Ii8xMjAwMjg3MDMvNTEwMTM0OTIxLTZjM2ZhNTQwLTBhMmQtNGExZC1hNDBlLTMxOWI1YmY4MTBkNC5naWY_.R2YYUG4J4pzCrsLyPH4VMrs6wn-hVcS7DP4_Sapo3HE)

---

## Les 5 - Debuggen en probleemoplossing

**Opdracht:**  
Gebruik de issues tracker om problemen in je Unity-project op te sporen en op te lossen.

🔗 [Tower Defence Game - Problemen](https://github.com/Ykada/TowerDefencegame/issues)

---

## Les 6/7 - SRP

**Opdracht:**  
Pas vaardigheden uit vorige lessen toe in een nieuwe gameproject-omgeving.

🔗 [Space48 Project](https://github.com/Ykada/Space48)

## Les 8

IDK

![510644945-f675f5ab-b7aa-4c4b-82d4-d4354b6e9eb7](https://github.com/user-attachments/assets/1404d081-7de1-4bad-a7a8-9377c573ec38)
